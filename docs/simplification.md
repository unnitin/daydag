...
| `observe.py` | `smoke` + `runlog` | one row type (`Result`) from the probe to the projection; the pre-flight over injected probes; the run row; the projection line, now written into State.md. `soak` keeps its own module: a gate journal is not observability |
| `registry.py` | `registry` + `manifests` | the checks and the reader off disk in one place; `python -m daydag.registry` |
| `delivery.py` | `delivery` | `Transport`, `deliver_push`, `deliver_prep_ping`, `draft` - the shape the guardrails inspect; its run-log rows are the shared `Result` |
| `cli.py` | four `__main__` blocks | one argparse tree; `python -m daydag.run` etc. stay as three-line entries (#117, #126) |
