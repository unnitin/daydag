...
| Atlassian / Jira | read; comments + transitions as proposals only | **plan of record for the data team** — sprint/board state, ticket status, assignee, epic rollup; the "is that on the board?" answer; evidence for chase-list loops that name a ticket | projects + JQL saved in `DayDAG/Watchlist.md`; ticket key (`ABC-123`) is the join key to Slack/PR/meeting-note mentions. **Authorized Sep 6, 2026.** The token is `read:jira-work` only - no write scope exists, so guardrail 4 is enforced at the token rather than by convention. Live board is `CDI`; see [reference/connector-audit.md](reference/connector-audit.md) |
...
- **No platform observability or evals.** Failure mode is Nitin noticing a brief didn't arrive. Phase 2 should therefore write a one-line run log (timestamp · loop · sources reached · sources skipped) to `DayDAG/State.md` so a silent failure is at least diagnosable after the fact. *(Status 2026-09-18: the row is written to the event log by `runlog`; the State.md line is still appended by hand at the end of each run.)*
