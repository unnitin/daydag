"""The morning brief (SPEC 3.1) - the day's first push, and the fullest one.

USING IT
    sources = YourSources()     # `push.Sources` is a Protocol - implement it.
                                #   The brief itself reads four of its methods:
    ...
    red_items(note)             # -> [(line number, text)] under a red heading
...
CONTRACTS
    ...
    2. Evidence or silence (guardrail 3). Every claim renders with a permalink
       ...
       `push.unsourced_claims` makes that mechanical rather than advisory.
    3. Degrade, never stall (guardrail 6). A source that raises costs one
       "couldn't check X" line and the brief still ships.
    4. Sources are INJECTED, as an object of methods - not as data. The brief
       has to observe HOW it asked (one calendar day at a time, an id-scoped
       Slack query), because those are the two failures that come back as a
       plausible empty result rather than an error. A client built in here
    ...
    6. Two answers to a naive datetime, on purpose. A naive EVENT START is
       read as Pacific (`push.local`) - the connector hands back wall-clock
       time and the alternative is worse. A naive ``now`` is REFUSED
       (`push.aware`, raising `PushError`) - the overnight cutoff is an hour of
       his day, and guessing which day is not a thing to do quietly.
...
WHY IT EXISTS
    It owns no source, no query and no rendering. Windows, cutoffs and vault
    paths come from `recipes`; the push shape, the degrade path and every
    citation helper from `push`; notes gaps from `ledger`; shipping from
    `pulse`; chase and watch from `DayDAG/State.md` via `statedoc`; the
    register from `voice`.
...
def assemble(
    *,
    ...
    if ledger is not None:
        ...
        # to an aware `now`, so a connector that omits a key or hands back a
        # naive datetime would take the whole brief down over the one section
...
def _is_overnight(record: Mapping[str, Any], window: recipes.OvernightWindow) -> bool:
    ...
    Reads both bounds off the window they were computed with, so the filter and
    the definition cannot drift apart. Both ends matter - see
    `recipes.OvernightWindow.max_ts` for the run that reported its own future
    (test_a_message_from_after_now_is_not_overnight).
