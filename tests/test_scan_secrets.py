...
@pytest.mark.parametrize(
    "text,label",
    [
        ("dm target ${SLACK_USER_PRINCIPAL}", "slack user id"),
        ("in:<#${SLACK_CH_POD_DISCOVERY}>", "slack channel id"),
        ("DM Sponsor ${SLACK_DM_SPONSOR}", "slack dm id"),
        ("doc ${GDOC_GOALS_PRIORITIES}", "google file id"),
        ("user ${NOTION_USER_JONATHAN}", "uuid (notion/atlassian)"),
        ("profile ${DATABRICKS_WORKSPACE_ID}", "databricks workspace id"),
        ("mail ${EMAIL_VP_DATA}", "internal email"),
...
def test_reports_line_numbers():
    ((lineno, label, value),) = scan_text("clean\nleak ${SLACK_USER_PRINCIPAL}\n")
    assert (lineno, label, value) == (2, "slack user id", "${SLACK_USER_PRINCIPAL}")
